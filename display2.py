...
class GaugeDisplay(object):
    ...
    def render(self, at):
        ...
        surf.fill(BASE2)
        # draw bar
        ratio = self.gauge.current(at) / float(self.gauge.top)
        if ratio > 1:
            bar_color = BLUE
        ...
        if ratio > 0:
            bar = pygame.Surface(
                (int(self.size[0] * max(0, min(1, ratio))), self.size[1]))
            bar.fill(bar_color)
            surf.blit(bar, (0, 0))
        # write current state
        text = font.render(
        ...
        if effects and move_in:
            text = font.render('+1 in {0:.0f}s'.format(math.ceil(move_in)), True, text_colors[1])
            surf.blit(text, (surf.get_width() - text.get_width() - 10,
                             font.get_height() / 2))
        '''
        try:
            move_in = .move_in(self.gauge, at)
...
if __name__ == '__main__':
    ...
    g1.add_momentum(Linear(+0.0329, 1))
    g1.add_momentum(Linear(+1, 30))
    main([g1])#, g2])
